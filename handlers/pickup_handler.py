...
from models.resolvers.auth_resolver import initializeGraphql, initialize_auth
...
@router.message(RegisterMessage.user_token and F.text)
async def extract_data(message: Message, state: FSMContext):
    ...
    await state.set_state(RegisterMessage.user_me)
    if botMessage.user_token:
        result = await initialize_auth(message.text, message.chat.id)
        await message.answer(text=result)
    stateSS = await state.get_state()
    print('stateSS', stateSS)


@router.message(RegisterMessage.user_me)
async def connect_websocket(message: Message, state: FSMContext):
    await message.answer('graphql subscription')
